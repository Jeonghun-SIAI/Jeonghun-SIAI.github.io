var store = [{
        ...
        "title": "[작성중] 선형계획법 기반 분산에너지시스템 최적화 - 7) 전기와 열의 수요/공급을 동시에 고려 (섹터커플링)",
        "excerpt":"지금까지는 전기에 대해서만 다룸, 그러나 건물의 경우 난방과 냉방을 쓰고, 지역에너지의 경우에도 지역난방 등이 있음 최근에는 ‘섹터커플링’이란 이름으로 주목받고 있음, 전기를 열, 수소 등 다른 에너지 형태로 상호 변환 이번 포스팅에서는 전기와 열 수요-공급을 동시에 고려하는 경우 소개 가장 간단한 케이스: 전기는 일반전기부하+에어컨냉방부하, 열은 보일러로 공급 이건 섹터커플링은 아님, 전기...","categories": ["optimalsystem"],
        "tags": ["선형계획법","부하패턴","경제성분석","Python"],
        "url": "/linprogseven.html",
        "teaser": null
      },{
        "title": "[작성중] 선형계획법 기반 분산에너지시스템 최적화 - 8) 출력조정 관련 제약들",
        "excerpt":"분산형에너지 패러다임 이전의 전통적 발전 포트폴리오에서도, 원자력/ 석탄/ 가스/ 유류 등 다양한 발전기 믹스를 구성한 이유 - 발전기별로 연료단가와 부하추종 성능에 차이가 있기 때문 부하추종 성능은 낮으나 연료단가도 낮은 설비는 기저설비로, 연료단가는 높으나 부하추종 성능은 높은 설비는 첨두설비로 시간대별 발전 포트폴리오. 일정 출력을 유지하는 기저설비, 특정 시간대에만 가동되는 첨두설비로 구성됨....","categories": ["optimalsystem"],
        "tags": ["선형계획법","부하패턴","경제성분석","Python"],
        "url": "/linprogeight.html",
        "teaser": null
      },{
        "title": "[작성중] 선형계획법 기반 분산에너지시스템 최적화 - 9) '부분'부하 성능",
        "excerpt":"발전기의 효율, 냉난방설비의 COP (성능계수) 를 상수로 가정했었음 그러나 실제로는 부하율에 따라 달라짐 발전기의 경우 아래 그림처럼 낮은 부하에서 효율이 저하된다. 열병합발전기의 통상적인 부하율-효율 그래프. 낮은 부하율에서 효율이 저하됨. 출처: “Modeling of non-linear CHP efficiency curves in distributed energy systems” 반대로 (중대형) 냉방기기의 경우 아래 그림처럼 낮은 부하에서 COP가 더...","categories": ["optimalsystem"],
        "tags": ["선형계획법","부하패턴","경제성분석","Python"],
