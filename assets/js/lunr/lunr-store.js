var store = [{
        ...
        "excerpt":"의사인 절친이 PubMed 사이트에서 수동으로 논문을 수집하고 있길래, 총합 두 시간 정도를 써서 Python으로 아래의 web scrapper를 만들어 준 적이 있다. 아래의 web scrapper로 검색어에 대응하는 논문들의 URL/ 출판일/ 저자/ 소속/ 키워드/ 저널명 수집이 가능하다. 특히 검색 option을 포함한 상세 검색도 가능하다. 또한 $\\texttt{urlopen}$ 라이브러리를 사용했기에, $\\texttt{requests}$ 라이브러리를 통한 크롤링이...","categories": ["etc"],
        "tags": ["크롤링","Python"],
        "url": "/scrapper.html",
        "teaser": null
      },{
        ...
        "excerpt":"논문 배경 필자는 박사과정 동안 수행했던 연구들로, 국제학술지 Applied Energy에 제1저자 논문 2편을 게재하였다. 그 중 첫 번째 논문의 주제는 ‘태양광/ 풍력/ ESS를 포함한 고립계통에서 재생에너지 출력제한(curtailment)의 총량에 상한을 둘 경우’에 대한 최적화 문제 세팅 및 분석이다. (조금 자기PR 하자면, Applied Energy의 Impact Factor는 2020년 이전에도 7~8이었고 지금은 10이 넘으며,...","categories": ["optimalsystem"],
        "tags": ["녹색섬","출력제한","선형계획법"],
        "url": "/mypaperone.html",
        "teaser": null
      },{
        "title": "내 Applied Energy 논문의 수학 설명 - 도심지 건축물 신재생 설치 확대 영향을 '빨리' 분석하기",
        "excerpt":"논문 배경 필자는 박사과정 동안 수행했던 연구들로, 국제학술지 Applied Energy에 제1저자 논문 2편을 게재하였다. 그 중 두 번째 논문의 주제는 ‘건축물에 설치되는 신재생에너지의 보급 확대 시의 도시 단위 영향 추정’을 보다 적은 계산비용으로 수행하는 방법이다. 이 포스팅은 해당 논문에서 계산 결과를 얻는 데 사용한 직관과 수학 내용을 정리한 것이다. (조금...","categories": ["optimalsystem"],
        ...
        "excerpt":"이전 포스팅에서 봤던 건물 내 배터리 스케줄링 문제에서는, 목적함수와 제약조건식들이 모두 선형이었다. 제곱항, 로그항 등은 없었으며, 두 개의 변수들 간의 곱도 없었다. 이 경우, 선형계획법 (Linear Programming) 을 이용해서 풀 수 있다. 선형계획법을 ‘무료로’ 수행하기 위해서는, Python에서 cvxopt 패키지 설치 후 glpk 모듈을 쓸 수 있다 (설치 방법 링크). 무료인...","categories": ["optimalsystem"],
        "tags": ["선형계획법","부하패턴","경제성분석","Python"],
        "url": "/linprogthree.html",
        "teaser": null
      },{
        "title": "정수선형계획법 기반 분산에너지시스템 최적화 설명 - 4) 태양광과 배터리의 용량 정하기",
        "excerpt":"지난 포스팅에서는 `주어진 용량의’ 배터리가 건물에 설치된 경우의 시간별 최소비용 충/방전 스케줄을 구하기 위한 선형계획법 문제 수식 구성 및 코딩을 설명했다. 그런데, 실제로는 그 용량도 결정해야 한다. 그래서 이번 포스팅에서는 용량까지 결정하는 문제를 설명한다. 더불어, 건물에 배터리 뿐 아니라 태양광 패널도 설치해서 태양광+배터리 연계 시스템을 구축 시 각각의 용량을 결정하는...","categories": ["optimalsystem"],
        "tags": ["선형계획법","부하패턴","경제성분석","Python"],
        ...
        "excerpt":"필자에게 있어 각별한 데이터가 있다. (비주거용) 건축물 별 월별 에너지 사용량 데이터다. 각 지번 주소 단위의 개별 비주거용 건물별로 특정 월에 소비한 전기와 가스의 양을 kWh 단위로 기록한 데이터다. 어떤 건물의 1년 12개월 간 월별 전기 사용량과 가스 사용량은, 대부분 아래 그림과 같은 형태를 띤다. 특정 건물의 1월부터 12월까지의 전기...","categories": ["dataset"],
        ...
        "excerpt":"이전 포스팅에서, 각 건물의 지번별/월별 전기와 도시가스 사용량 데이터를 소개하였다. 또한 이를 표제부와 결합해 건물 에너지 분석에 활용할 수 있으며, 그 결합 방법 또한 소개하였다. 그렇다면 결합된 데이터를 그대로 쓰면 되는가? 그렇지 않다. 분명히 ‘이상한’ data point들이 존재할 것이기 때문이다. 데이터 기반의 연구개발을 한다면, 이러한 ‘이상한’ data point들을 제거하거나 수정하는...","categories": ["dataset"],
        "tags": ["건물 에너지 데이터","회귀분석"],
        "url": "/monthlyenergytwo.html",
        "teaser": null
      },{
        "title": "건축물 별 월별 에너지 사용량 데이터셋 - 3) 월별 사용량 크기가 이상한 data point 제거",
        "excerpt":"저번 포스팅에서는 건물 월별 에너지 사용량의 ‘추이’가 이상한 data point를 판별하는 방법을 설명했다. 이번 포스팅에서는 월별 에너지 사용량의 ‘크기(magnitude)’가 이상한 data point를 판별하는 방법을 설명한다. 에너지 사용량 크기 측면에서의 outlier 상식적으로, 같은 용도의 건물이라면 크기가 큰 건물일수록 에너지 사용량이 큰 경향이 있을 것이다. 이를테면 서울 내 업무용 건물들의 1월 전기...","categories": ["dataset"],
        "tags": ["건물 에너지 데이터","회귀분석"],
